  ...
  return (
    <div className="min-h-screen bg-paper-100">
      ...
      <main className="mx-auto max-w-3xl px-6 pb-24 pt-6">
        ...
        <h1 className="mt-4 font-serif text-4xl text-ink-950">Here's your score</h1>
        <p className="mt-2 max-w-lg text-ink-600">
          This is the same, complete score companies will see when you express interest —
          nothing is hidden or summarized differently on their side.
        </p>
          ...
          <div className="p-8 sm:p-10">
              ...
              {SCORE.dimensions.map((d) => (
                <div key={d.id}>
                  <div className="flex items-baseline justify-between">
                    <span className="font-serif text-lg text-ink-950">{d.label}</span>
                    <span className="font-serif text-lg text-ink-950">{d.score}<span className="text-sm text-ink-400">/100</span></span>
                  </div>
                  <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-ink-900/8">
                    <div className="h-full rounded-full bg-verified-600" style={{ width: `${d.score}%` }} />
                  </div>
            ...
            <div className="mt-8 flex items-start gap-3 rounded-sm border border-verified-700/25 bg-verified-600/8 px-5 py-4">
              ...
              <p className="text-sm leading-relaxed text-verified-700">
                <span className="font-semibold">Full transparency:</span> companies will see this exact
                score — the same three dimensions, the same percentile, the same summaries. No black box.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 flex justify-end">
          <PrimaryButton onClick={onSeeMatches}>
            See your unlocked roles <span aria-hidden>→</span>
          </PrimaryButton>
        </div>
      </main>
    </div>
